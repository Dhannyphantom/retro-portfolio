      ...
      <div className="grid grid-cols-1 md:grid-cols-[1.1fr_0.9fr] gap-12 items-center mb-[52px]">
        <Reveal from="left">
          <div className="grid grid-cols-3 gap-3.5">
            {photos.map((p, i) => (
              <div key={p.caption || i} className="group relative rounded-[10px] overflow-hidden aspect-[4/5]">
                ...
                {p.caption && <div className="absolute bottom-2 left-2.5 text-[11.5px]">{p.caption}</div>}
              </div>
            ))}
          </div>
        </Reveal>
        <Reveal from="right" delay={80}>
          <p className="text-mute text-[15px] leading-[1.75]">{bio}</p>
        </Reveal>
      </div>
        ...
        {videos.map((v, i) => (
          <Reveal key={v.title} delay={i * 90}>
            ...
            >
              ...
              {v.duration && <span className="absolute bottom-2.5 right-2.5 text-[11px] font-mono px-1.5 py-1 rounded" style={{ background: "rgba(7,5,11,0.55)" }}>{v.duration}</span>}
