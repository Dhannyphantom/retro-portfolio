...
export default function Philosophy({ lines = FALLBACK }: { lines?: string[] }) {
  return (
    <section className="max-w-[900px] mx-auto px-7 py-16 text-center">
      <Reveal>
        <span className="font-mono text-xs tracking-widest text-violet">$ PHILOSOPHY</span>
      </Reveal>
      <div className="mt-[22px] flex flex-col gap-1">
        {lines.map((line, i) => (
          <Reveal key={line} delay={i * 90}>
            <p className={`font-display font-semibold text-[clamp(12px,1.8vw,16px)] my-2.5 tracking-tight leading-relaxed ${i % 2 === 0 ? "text-paper" : "text-violet"}`}>{line}</p>
          </Reveal>
        ))}
      </div>
    </section>
