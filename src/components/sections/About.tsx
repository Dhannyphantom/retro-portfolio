      ...
      <div>
        {SERVICES.map((s, i) => (
          <div key={s.title} className={`flex gap-[18px] py-[22px] ${i !== 0 ? "border-t border-white/[0.07]" : ""}`}>
            <div className="w-0.5 flex-shrink-0 bg-gradient-to-b from-neon to-phosphor" style={{ height: 40 }} />
            <div className="icon-hover w-10 h-10 rounded flex items-center justify-center flex-shrink-0 text-neon" style={{ border: "1px solid rgba(0,229,255,0.122)", background: "rgba(57,255,20,0.034)" }}>
              <s.icon size={17} />
            </div>
            <div>
      ...
      <div>
        <Reveal from="right">
          <div className="flex items-center gap-2.5 mb-[18px]">
            <div className="w-0.5 h-[22px] bg-neon" />
            <span className="font-mono text-xs tracking-widest text-neon">$ INTRODUCE</span>
          </div>
          ...
          <p className="text-mute text-[13px] leading-[1.7] max-w-[440px] mb-4">
            Don&apos;t just take my word for it — the terminal below actually runs. Type <code className="text-neon">help</code> to
